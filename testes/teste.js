...
async function testeEtherNetIP() {
    ...
    // O protocolo EtherNet/IP utiliza um "Session Handle", que é utilizado como uma chave que você precisa informar em suas solicitações CIP para o dispositivo remoto reconhecer que você está autorizado a fazer solicitações.
    ...
    // Inicialmente, vamos solicitar um comando pra pegar as informações basicas do dispositivo como nome, IP, versão, etc..(isso não é um comando relativo ao protocolo CIP e sim algo do protocolo EtherNet/IP que todo dispositivo tem implementado)

    // O comando que vamos solicitar é o "List Identity" do EtherNet/IP, que retorna informações basicas que descrevem o dispositivo. A função build irá configurar o layer automaticamente com as informações necessarias.
    ...
    // A operação é considerada somente sucesso se a requisição conseguiu ser enviada ao dispositivo E o dispositivo respondeu com sucesso de volta.
    if (!aguardaResposta.isSucesso) {

        // Aqui podemos tratar o erro e saber o que aconteceu
        if (!aguardaResposta.enipEnviar.isEnviou) {

            // Se isEnviou for false, isso indica que nem sequer foi enviada ao dispositivo. Ou não há conexão com o dispositivo, ou o pacote ENIP tem algum erro.

            // Se isGerarBuffer for true, então o erro ocorreu ao gerar o buffer do pacote ENIP. A causa disso pode ter sido em escrever em um offset invalido, algum valor invalido informado em algum campo que não coube no Buffet, etc..
            if (aguardaResposta.enipEnviar.erro.isGerarBuffer) {
                ...
                // O trace log é um array de mensagens que descreve em ordem exatamente a sequencia de eventos da geração do Buffer, com ele é possível saber exatamente onde ocorreu o erro.
        ...
        if (!aguardaResposta.enipReceber.isRecebeu) {
            ...
            // Por padrão, se o dispositivo não responder a solicitação em 9000ms, a solicitação é considerada como "demorou" e o pacote é descartado.
    ...
    // Vamos extrair as informações do pacote recebido utilizando o Parser. O Parser é responsável por interpretar o pacote recebido e extrair as informações que queremos. 
    let parserENIPResposta = aguardaResposta.enipReceber.enipParser;

    // Verificar se a resposta recebida é valida(as informações do Buffer recebidas estão corretas). Se não for, então o pacote recebido está corrompido e não deve ser considerado.
    if (!parserENIPResposta.isValido().isValido) {

        // Se não for valido, o Buffer recebido está corrompido e não deve ser considerado. isValido retorna a descrição em qual parte do Buffer deu erro e também tem o Tracer que é um array de mensagens que descreve exatamente a sequencia de eventos que ocorreu até chegar no erro.
    ...
    // Lembrando que isValido só verifica se o Buffer recebido está correto. O pacote pode ter sido recebido corretamente, mas a resposta do dispositivo pode ter sido um erro devido a algum comando ilegal. Pra isso existe o comando de getStatus também.

    // Ok, se o Buffer é valido, agora precisamos verificar se o status da solicitação é valido, já que pode ter ocorrido algum erro interno no dispositivo ou alguma informação que não foi possivel ser processada.
    ...
    // Se o Buffer é valido e com status sucesso, podemos extrair as informações do pacote recebido para o comando que desejamos.

    // Como enviamos um comando ListIdentity, é esperado que a resposta seja também um ListIdentity. Na teoria se você enviou um comando X, ele nunca deve retornar outra coisa além de X, mas sei lá coisas estranhas podem acontecer.
    ...
    // Como é um ListIdentity, chamamos o parser do ListIdentity que irá pegar o Buffer recebido e extrair as informações do ListIdentity.
    const parserListIdentity = parserENIPResposta.getAsListIdentity();

    // Assim como fizemos assim, precisamos validar se o Buffer recebido do ListIdentity é valido e não contém informações corrompidas.
